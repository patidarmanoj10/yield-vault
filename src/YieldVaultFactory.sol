 ...
 * @dev Supports both upgradable (proxy) and non-upgradable (direct) vault deployments
...
contract YieldVaultFactory is Ownable2Step {
    ...
    /// @notice Emitted when a new non-upgradable vault is created
    /// @param vault Address of the newly created vault
    /// @param name Name of the vault token
    /// @param symbol Symbol of the vault token
    /// @param asset Address of the underlying asset
    /// @param creator Address that created the vault
    event NonUpgradableVaultCreated(
        address indexed vault,
        string name,
        string symbol,
        address indexed asset,
    ...
    error AddressIsNull();
    error NonUpgradableVaultNotSupported();
    ...
    /**
     * @notice Creates a new YieldVault (upgradable or non-upgradable)
     * @param name_ Name of the vault token (e.g., "Yield Vault ETH")
     * @param symbol_ Symbol of the vault token (e.g., "yvETH")
